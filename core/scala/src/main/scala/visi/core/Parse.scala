...
import util.parsing.input.{NoPosition, Positional, Reader}
 ...
 */
class Parse extends JavaTokenParsers {
  ...
  protected lazy val literalExpr: Parser[Expression] = this.stringLiteral ^^ {
    case s => ValueConst(calcLoc, StrValue(s), TPrim(PrimStr))
  }
